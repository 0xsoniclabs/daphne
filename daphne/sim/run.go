...
func loadScenario(c *cli.Command) scenario.Scenario {
	...
	return &scenario.DemoScenario{
		...
		Broadcaster: getBroadcastFactory(c),
	}
}

func getBroadcastFactory(c *cli.Command) broadcast.Factories {
	protocol := c.String(broadcastProtocolFlag.Name)
	...
	case "gossip", "g":
		...
		return factory
	default:
		slog.Warn("Unknown broadcast protocol in configuration, using defaults", "protocol", protocol)
		return factory
	}
}
