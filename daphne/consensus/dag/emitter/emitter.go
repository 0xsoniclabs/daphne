...
type Channel interface {
	// Emit signals the emission of a new event with the given snapshot of the DAG heads
	// on which the new event will be based. The snapshot provided represents the exact
	// state of the DAG heads upon which the the emission conditions were evaluated.
	Emit(dagSnapshot map[consensus.ValidatorId]*model.Event)
}
