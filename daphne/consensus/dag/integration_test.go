...
func testDagConsensus_ThreeNodes_ConsistentlyLinearizesTransactions(t *testing.T, layeringFactory layering.Factory) {
	...
	active1TxSource.EXPECT().GetCandidateTransactions().Return(active1Lineup).AnyTimes()
	...
	active2TxSource.EXPECT().GetCandidateTransactions().Return(active2Lineup).AnyTimes()
