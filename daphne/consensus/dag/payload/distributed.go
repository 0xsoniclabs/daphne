...
type DistributedProtocol struct {
	...
	// highestNonceProposed tracks, per sender, the highest nonce that this
	// validator has proposed so far. This is used to avoid re-emitting
	// transactions with nonces lower than or equal to the highest nonce
	// proposed.
...
func (p DistributedProtocol) isMyResponsibility(
	...
	data := []byte{}
	data = binary.BigEndian.AppendUint32(data, round/NumRoundsBetweenReassignments)
