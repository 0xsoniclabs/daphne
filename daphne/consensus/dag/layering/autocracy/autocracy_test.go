...
func TestAutocracy_NewLayering_ErrorOnEmptyCommittee(t *testing.T) {
	_, err := (&AutocracyFactory{}).NewLayering(model.NewDag(), map[model.CreatorId]uint32{})
	require.ErrorContains(t, err, "empty committee")

	_, err = (&AutocracyFactory{}).NewLayering(model.NewDag(), nil)
	require.ErrorContains(t, err, "empty committee")
}

func TestAutocracy_NewLayering_ErrorOnNilDag(t *testing.T) {
	_, err := newAutocracy(nil, map[model.CreatorId]uint32{1: 1}, 0)
	require.ErrorContains(t, err, "nil DAG")
}
...
func TestAutocracy_NewAutocracy_CorrectlyInitializes(t *testing.T) {
	...
	autocracy, err := newAutocracy(model.NewDag(), map[model.CreatorId]uint32{1: 1, 2: 1}, candidateFrequency)
	require.NoError(err)
...
func TestAutocracy_IsLeader_ReturnsTrueForAutocrat(t *testing.T) {
	...
	for event, expected := range tests {
		t.Run(fmt.Sprintf("%+v", *event), func(t *testing.T) {
			isLeader, err := autocracy.IsLeader(event)
			require.NoError(err)
