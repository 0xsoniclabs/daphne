...
func TestMysticeti_CoverLine150(t *testing.T) {
	committee, err := consensus.NewCommittee(map[consensus.ValidatorId]uint32{
		consensus.ValidatorId(1): 1,
		consensus.ValidatorId(2): 1,
		consensus.ValidatorId(3): 1,
...
func TestMysticeti_FindAnchor_SkippedLeaders(t *testing.T) {
	...
	// Build structure where some leaders are skipped
	events := make([][]*model.Event, 10)
	...
	// Round 1: Skip the leader (validators[1])
	events[1] = make([]*model.Event, 4)
	skippedLeader := validators[1%len(validators)]
	for i, v := range validators {
		var parents []*model.Event
		if v == skippedLeader {
			// This is the leader, but we won't create their event yet
			continue
		} else if i == 1 {
			// Skip creating event for leader position
			continue
		} else {
			// Other validators create events without leader
			parents = []*model.Event{events[0][i], events[0][(i+1)%4]}
		}
		_, err := model.NewEvent(v, parents, nil)
		require.NoError(t, err)
	...
	// Round 2: validators[2] is leader, but skip it
	events[2] = make([]*model.Event, 3)
	for i, v := range validators {
		if i == 2 {
	...
	// Continue building to create anchors
	// Round 3: validators[3] is leader - create skip pattern
	...
	// This should trigger findAnchor which will skip over missing/skipped leaders
	leader0 := events[0][0]
	verdict := m.IsLeader(leader0)
	require.Contains(t, []layering.Verdict{layering.VerdictYes, layering.VerdictNo}, verdict)
}
